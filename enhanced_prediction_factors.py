...
class EnhancedPredictionFactors:
    ...
    def get_team_position(self, team_id, league_id=None):
        ...
        # Takım ID kontrolü
        if team_id is None:
            return 10  # Varsayılan orta sıra pozisyonu
        ...
        if not league_id:
            ...
            if str(team_id) in ["610", "614", "611", "636", "1005"]:  # Türk takımları
                ...
                try:
                    ...
                    if team_id_int < 100:  # İngiltere tahmin
                        league_id = "39"  # Premier Lig
                    elif team_id_int < 200:  # İspanya tahmin
                        league_id = "140"  # La Liga
                    elif team_id_int < 300:  # İtalya tahmin
                        league_id = "135"  # Serie A
                    elif team_id_int < 400:  # Almanya tahmin
                        league_id = "78"  # Bundesliga
                    elif team_id_int < 500:  # Fransa tahmin
                        league_id = "61"  # Ligue 1
                    else:
                        # Varsayılan lig
                        league_id = "203"  # Varsayılan olarak Türkiye Süper Lig
                except (ValueError, TypeError):
                    # Geçersiz team_id, varsayılan lig kullan
                    league_id = "203"
    ...
    def adjust_score_prediction(self, base_home_goals, base_away_goals, enhanced_factors):
        ...
        if "match_importance" in enhanced_factors:
            ...
            # MAÇTA SABİT ÇARPAN DEĞERLERİ KALDIRILDI
            # 1.1, 1.2, 0.1 gibi sabit çarpanlar kaldırıldı
            # Dinamik hesaplama sistemi kullanılmalı
            logger.info("Maç önemi sabit çarpanları kaldırıldı")
