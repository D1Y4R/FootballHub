...
# İY/MS modülü kaldırıldı - artık kullanılmıyor
    ...
    try:
        ...
        if prediction:
            ...
            normalized_response = {
                ...
                "away_win": prediction.get('predictions', {}).get('away_win_probability', 0),
                "most_likely_score": prediction.get('predictions', {}).get('exact_score', {}).get('prediction', 'N/A'),
                "over_2_5": prediction.get('predictions', {}).get('betting_predictions', {}).get('over_2_5', {}).get('probability', 0),
                "under_2_5": prediction.get('predictions', {}).get('betting_predictions', {}).get('under_2_5', {}).get('probability', 0),
                "both_teams_score": prediction.get('predictions', {}).get('betting_predictions', {}).get('both_teams_score', {}).get('probability', 0),
                "confidence": prediction.get('predictions', {}).get('confidence', 0),
                "home_team": {
                    "name": home_name,
                    "form": prediction.get('home_team', {}).get('form', {})
                },
                "away_team": {
                    "name": away_name,
                    "form": prediction.get('away_team', {}).get('form', {})
                },
                "match_analysis": prediction.get('predictions', {}).get('analysis', {}),
                "timestamp": time.time()
            }
...
def get_htft_prediction(home_team_id, away_team_id):
    ...
    Bu endpoint, Python tabanlı backend tahmin modelini kullanır
    ...
    try:
        ...
        if expected_home_goals is not None and expected_away_goals is not None:
            logger.info(f"Tahmin butonundan beklenen gol değerleri alındı: Ev={expected_home_goals:.2f}, Deplasman={expected_away_goals:.2f}")
        ...
        # İY/MS modülü kaldırıldı - basit yanıt döndür
        htft_prediction = {
            "prediction": "İY/MS modülü devre dışı",
            "probabilities": {},
